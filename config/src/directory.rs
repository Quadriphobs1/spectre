...
pub fn find_project_root() -> Result<PathBuf> {
  ...
  search_if_dir_is_root(&cur_dir, "Cargo.toml")
}

pub fn search_if_dir_is_root(path: &Path, file: &str) -> Result<PathBuf> {
  ...
  if toml_path.is_file() {
    ...
    path
      .parent()
      .map(|p| search_if_dir_is_root(p, file))
...
mod tests {
  ...
  use super::{find_project_root, search_if_dir_is_root};
  use std::env;
  use std::fs;
  ...
  fn find_cargo_toml() {
    ...
    assert_eq!(
      Ok(temp_path.clone()),
      search_if_dir_is_root(&temp_path, "Cargo.toml")
    );
    dir.close().unwrap();
  }
